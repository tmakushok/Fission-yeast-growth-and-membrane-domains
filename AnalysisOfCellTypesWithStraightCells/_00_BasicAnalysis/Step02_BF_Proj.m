...
for i_t = 1:TimePts - 1         % Loop on time points of the movie
    close all;
...
%         break
%     end
    MidPlanesAll(i_t + 1) = MiddlePlane;       % Save position of the middle plane for fluo projections
...
%% Do projection close to the middle plane
    FileName = [Directory Files(i_t * SlicesTotal + max(MiddlePlane - 2, 1)).name];     
    InitImageRGB = double(imread(FileName)); 
