...
QuestionCells = [5, 25, 31, 41, 45, 57, 61, 64];

PeaksWidths = cell(length(AllKymos), 1);
for i = 2:length(QuestionCells)       % Loop on cells
...
%     close all;
    ...
    for i_t = max(TimeStartGrowth{i_cell}{GrowEnd}(3),1):size(AllContours, 2)       % From the moment of cell growth initiation till the end of the movie
        close all;
