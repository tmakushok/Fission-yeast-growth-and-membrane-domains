...
clear;
%--------------------------------------------------------------------------
OutFile = '_Output/_QuickEndPatchWidthOverTime.mat';
OutEndsFile = '_Output/_QuicklyGrowingCellEnds.mat';
%--------------------------------------------------------------------------
...
%     close all;
    ...
    %% Choose the cell end corresponding to the quick growth (only in 'monopolar' cells)
    ...
    for i_t = TimeStartGrowth{i_cell}{GrowEnd}(3):size(AllContours, 2)       % From the moment of cell growth initiation till the end of the movie
        close all;
...
%             figure, plot(Kymo_t, '-*'), grid on;
        ...
        X_end = CellEnds(1 + 2*(GrowEnd - 1));
        Y_end = CellEnds(2 + 2*(GrowEnd - 1));
...
%% Save the number of the cell end considered as the slowly growing one
    QuickEnds(i_cell) = GrowEnd;
end
%% Output the result        
save(OutFile, 'PeaksWidths');
save(OutEndsFile, 'QuickEnds');
