...
load('Dynamics.mat'); 
load('SlowEndNumbers.mat'); 
load('PatchWidths_SlowEnd_FromCellGrInit.mat'); 

WidGr = [];
for i_cell = 1:length(Dynamics)
    if isempty(PeaksWidthsAll{i_cell})
       continue 
    end  
    % Finding which end is the "non-growing" one
    End = SlowEnds(i_cell);
    Dyn_OneEnd = Dynamics{i_cell}{End};
    ...
    for i_ph = 1:size(Dyn_OneEnd,1)       % Loop on phases of growth for the current cell        
        ...
        if t2 <= TimeInit    % If end of the phase is before cell started growing 
            continue
        end
        if t1 < TimeInit    % If the beginning of the phase is before cell started growing but the end of the phase is after that
            t1 = TimeInit;      % Do as if the phase started at the growth initiation
        end  
        for i = 1:t2-t1+1
