...
Dist = sqrt((Cont(:,1) - Center(1)) .^ 2 + (Cont(:,2) -  Center(2)) .^ 2);
plot(Dist), grid on;

%% Smoothing of the curve   
FitOptions = fitoptions('Method', 'SmoothingSpline', 'SmoothingParam', 0.01);   % Parameter 0.1 is too low
...
hold on, plot(Ends, DistSmoo(Ends), 'ko', 'MarkerSize', 11);