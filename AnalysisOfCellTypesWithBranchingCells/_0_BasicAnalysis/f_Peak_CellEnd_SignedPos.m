...
function [Peak] = f_Peak_CellEnd_SignedPos(Y, CEndPos)   % CEndPos is the position of the current cell end in coordinates of the intensity profile
%--------------------------
p_Spline = 0.01;
% PtsLinFit = 5;      % Nb of point for linear fits going along the profile
ThresPeak = 7;     % Smaller intensities are to be considered as non-important noisy values
...
%% Adding a "closing" end point to the kymo line, to avoid having peaks open at their right side
Y = [Y; 0];
...
Peak.DistFromCEnd = Peak.PositionOfPeakCenter - CEndPos;
