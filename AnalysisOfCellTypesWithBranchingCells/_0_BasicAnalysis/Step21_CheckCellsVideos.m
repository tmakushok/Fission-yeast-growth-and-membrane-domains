...
%% Defining the borders of the movie for this cell (from final cell outline)  
    ...
    Xmin = max(min(Coords(:,2)) - 45, 1);               % max - to account for image borders
    Ymin = max(min(Coords(:,1)) - 45, 1);   
    Xmax = min(max(Coords(:,2)) + 45, ImSize(2));       % min - to account for image borders
    Ymax = min(max(Coords(:,1)) + 45, ImSize(1));  
