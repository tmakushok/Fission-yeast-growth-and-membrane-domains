...
function [Peak] = f_Peak_CellEnd(Y, CEndPos)   % CEndPos is the position of the current cell end in coordinates of the intensity profile
%--------------------------
p_Spline = 0.01;
...
%% BkUp
% %% Find real values maximum (before smoothing)
% x_MP = max(1, MaxPos - ToLookForMax):MaxPos + ToLookForMax;
% [MaxProf, ind] = max(Y(x_MP));
% HalfHeight = MaxProf / 2;
% %% Threshold on the max intensity of the peak: if too small, not a proper
% %% peak
% if MaxProf < ThresPeak
%     Peak = [];  % There is no patch at the cell end
% end
% %% Width of the peak at half-height
% Diff = Y - HalfHeight;
% figure, plot(Diff, '-*'), grid on;
% % Going in Ox direction from peak max until finding first negative value
% ind1 = find(Diff(MaxPos:length(Y)) < 0);
% % In case ind1 are not found 
% if isempty(ind1)
%     ind1 = length(Y) - MaxPos + 1;
% end
% % Going in -Ox direction from peak max until finding first negative value
% ind2 = find(Diff(MaxPos:-1:1) < 0);
% % In case ind2 are not found 
% if isempty(ind2)
%     ind2 = MaxPos;
% end
% % '-1' because 'MaxPos' was counted twice
% Peak.Width = ind1(1) + ind2(1) - 3;    % Peak width at half-height  
% % Peak center position in coords of the outline
% % Calculated as the middle between the two points of peak width at
% % half-height
% Peak.PositionOfPeakCenter = MaxPos - ind2(1) + (ind1(1) + ind2(1)) / 2;     
% Peak.DistFromCEnd = abs(Peak.PositionOfPeakCenter - CEndPos);
% Peak.MaxIntens = MaxProf;   % Maximum intensity of the peak
